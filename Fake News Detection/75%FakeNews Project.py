...
dataset = pd.read_csv('75%Data.csv')

# dataset.subject.replace(to_replace="politicsNews", value='politics', inplace=True)
dataset.groupby(['subject'])['text'].count()

dataset.subject.value_counts().plot(kind="bar", color="m")

# dataset.notna().all(axis=0)
sns.heatmap(dataset.notna())
plt.show()
...
profile = ProfileReport(pd.read_csv('75%Data.csv'), title='Fake And True Profiling Report', explorative=True)

# Saving results to a HTML file
profile.to_file("75%output.html")
