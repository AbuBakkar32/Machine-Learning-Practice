...
religious = final_table['% Religious']
s_rate = final_table['Combined Suicide Rate']
plt.figure(figsize=(8, 6))
plt.title("Effect of Religion on Suicide Rates", fontsize=17, color="#073A5A")
plt.scatter(religious, s_rate)
plt.xlabel("% of population is Religious", fontsize=15, color="#073A5A")
plt.ylabel("Suicide rate %", fontsize=15, color="#073A5A")
...
sorted_by_rel = final_table.sort_values(by=['% Religious'], ascending=True)
sorted_by_rel.head(30)

bot50 = sorted_by_rel['% Religious'][:30]
bot50_suicide = sorted_by_rel['Combined Suicide Rate'][:30]
top50 = sorted_by_rel['% Religious'][-30:]
top50_suicide = sorted_by_rel['Combined Suicide Rate'][-30:]
avg_sui_bot = bot50_suicide.mean()
avg_sui_top = top50_suicide.mean()

f, (ax1, ax2) = plt.subplots(1, 2, sharey=True, figsize=(15, 5))

ax1.set_title("Bottom 30 Religious Countries Suicide %", fontsize=16)
ax1.set_xlabel("% of population is Religious", fontsize=14, color="#073A5A")
ax1.set_ylabel("Suicide rate %", fontsize=14, color="#073A5A")
ax1.scatter(bot50, bot50_suicide, color="red", label='Actual Data')
ax1.axhline(y=13.047, xmin=0, xmax=1, color='#FFC100')

bot_avg_patch = mpatches.Patch(color='#FFC100', label="AVG Suicide Rate = {}".format(round(avg_sui_bot, 3)))

ax1.legend(handles=[bot_avg_patch])

ax2.set_title("Top 30 Religious Countries Suicide %", fontsize=16)
ax2.set_xlabel("% of population is Religious", fontsize=14, color="#073A5A")
ax2.set_ylabel("Suicide rate %", fontsize=14, color="#073A5A")
ax2.scatter(top50, top50_suicide, color="blue")
ax2.axhline(y=8.482, xmin=0, xmax=1, color='#aaff80')

top_avg_patch = mpatches.Patch(color='#aaff80', label="AVG Suicide Rate = {}".format(round(avg_sui_top, 3)))

ax2.legend(handles=[top_avg_patch], loc="upper left")
...
test_table2 = test_table.drop(columns=['Rel_High', 'Rel_Low', 'Combined_suicide_rate'])
scaled_table = scale(test_table2)
scaled_table = pd.DataFrame(scaled_table, columns=[test_table2.columns])
scaled_table['Rel_High'] = np.array(test_table['Rel_High'])
scaled_table['Rel_Low'] = np.array(test_table['Rel_Low'])
scaled_table.head()
...
features_poly = pd.DataFrame(poly.fit_transform(scaled_table),
                             columns=poly.get_feature_names(test_table.drop(columns=['Combined_suicide_rate']).columns))
...
def MSE_test(X_poly, y, random_state=11):
    ...
    for i in range(1, len(X_poly.columns)):
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_poly[selected_columns], y, random_state=random_state,
                                                            test_size=.20)

        crossvalidation = KFold(n_splits=5, shuffle=True, random_state=1)
        cv_10_results = np.mean(
            cross_val_score(linreg, X_train, y_train, cv=crossvalidation, scoring='neg_mean_squared_error'))
...
X_train, X_test, y_train, y_test = train_test_split(X_poly[selected_columns], y, random_state=12, test_size=.20)
